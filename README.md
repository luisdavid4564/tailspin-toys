...
[`copilot-instructions.md`](.github/copilot-instructions.md) and the focused
guides in [`.github/instructions/`](.github/instructions/). In particular:

- Comments explain intent and non-obvious decisions, not mechanics already clear from the code.
- Exported functions in `db/` and `src/lib/` use TSDoc/JSDoc for their purpose, parameters, and return values.
- Reusable Astro components document their `Props` contracts.
- TypeScript uses consistent formatting that is checked by ESLint.
